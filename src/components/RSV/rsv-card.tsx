...
interface ReservoarCardProps {
  ...
  bgColor?: string;
  btnColor?: string;
  levelAirSebelumnya?: number;
  levelAirSekarang?: number;
  waktuCatat?: string | null;
  isPast?: boolean;
}

const RsvCard: React.FC<ReservoarCardProps> = ({
  title,
  levelAirSebelumnya,
  levelAirSekarang,
  onAction,
        ...
        <div className="w-24 aspect-[4/3] h-auto flex-shrink-0">
          <div className="w-full h-full overflow-hidden rounded-md">
            <img
              ...
              onClick={() => setOpen(true)} // buka lightbox pas klik
            />
          </div>
        </div>
        ...
        <div className="flex flex-col justify-between flex-1 text-xs ">
          <div>
            ...
            <p className="flex justify-between py-1 ">
              <span>Level Air Sebelumnya:</span>
              <span>
                {levelAirSebelumnya === 0 ? "-" : levelAirSebelumnya} cm
              </span>
            </p>
            <div className="my-1 border-b border-gray-300" />
            <p className="flex justify-between py-1 ">
              <span>Level Air Sekarang:</span>
              <span>{levelAirSekarang === 0 ? "-" : levelAirSekarang} cm</span>
            </p>
          </div>
