...
import { listPemakaianKantorQuery } from "../../../../queries/pemakaian-kantor/list-meter-pemakaian-kantor";
import { useAuthStore } from "../../../../store/auth";
import { useQuery } from "@tanstack/react-query";
import DetailPemakaianKantorCard from "../../../../components/PEMAKAIAN-KANTOR/pakai-kantor-detail-card";
...
function RouteComponent() {
  ...
  const kode = useParams({ strict: false }).kode;
  ...
  return (
    <div className="w-full p-4 bg-whiteCust rounded-xl">
      ...
      >
        ...
        Catat Pemakaian Kantor
      </button>
      {ListData?.data && ListData.data.length > 0 ? (
        ListData.data.map((value) => (
          <div className="mb-4" key={value.id}>
            <DetailPemakaianKantorCard
              image={value.file ? value.file : null}
              pemakaian={Number(value.st_akhir) - Number(value.st_awal)}
              namaMeter={value.nama_meter_produksi}
              standAwal={Number(value.st_awal)}
              standAkhir={Number(value.st_akhir)}
