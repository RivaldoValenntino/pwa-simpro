...
import SungaiCard from "../../../../components/SGAI/sgai-card";
...
function RouteComponent() {
  ...
  const {
    ...
    isLoading,
    isFetching,
  } = useQuery(listSungaiQuery(dateServer, kode, Number(id_installasi), jam));
  ...
  return (
    <>
      <div className="w-full p-4 overflow-hidden shadow-xs bg-whiteCust rounded-xl ">
        ...
        <div className="mt-12">
          {filteredData?.map((sgai) => (
            <SungaiCard
              isPast={isPastJam}
              waktuCatat={sgai.waktu_catat}
              btnColor={
                !sgai.level_sungai
              ...
              titleAction={
                sgai.level_sungai ? "Edit Level Sungai" : "Catat Level Sungai"
              }
              image={
                sgai.file === "" || sgai.file === null ? NoImage : sgai.file
              }
              key={sgai.id}
              title={sgai.nama_meter_produksi}
              levelSungaiSebelumnya={Number(sgai.level_awal) || 0}
              levelSungaiSekarang={Number(sgai.level_sungai) || 0}
            />
          ))}
        </div>
      </div>
